            ...
            {seciliPortfoy ? (
              <>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {[
                    { b: 'Toplam Değer', d: `₺${fmt(seciliPortfoy.toplamDeger)}`, r: 'text-gray-900' },
                    { b: 'Toplam Maliyet', d: `₺${fmt(seciliPortfoy.toplamMaliyet)}`, r: 'text-gray-900' },
                    { b: 'Nominal Getiri', d: `₺${fmt(seciliPortfoy.nominalGetiri)}`, r: seciliPortfoy.nominalGetiri >= 0 ? 'text-green-600' : 'text-red-500' },
                    { b: 'Getiri %', d: `${fmt(seciliPortfoy.nominalGetiriYuzde)}%`, r: seciliPortfoy.nominalGetiriYuzde >= 0 ? 'text-green-600' : 'text-red-500' },
                  ].map(({ b, d, r }) => (
                    <div key={b} className="bg-white rounded-xl border border-gray-200 p-4">
                      <p className="text-xs text-gray-500 mb-1">{b}</p>
                      <p className={`text-lg font-bold ${r}`}>{d}</p>
                    </div>
                  ))}
                </div>
